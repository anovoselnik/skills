...
Turn the supplied task into verified, reviewed commits. The user's requested scope and repository instructions govern the work.
...
5. Review all intended changes with the available `code-review` skill. Supply the recorded starting commit as the fixed point and the original task as the spec. If that skill requires a committed diff, make a scoped checkpoint commit first. Without the skill, review the complete diff against the task and repository standards directly.
