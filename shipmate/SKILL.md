...
Apply those limits before resolving dependencies or entering stages. "Plan only" runs no execution stages. "Do not commit" leaves the implementation uncommitted and skips publishing and monitoring; "keep it local" or "do not push" stops after local implementation; "skip monitoring" finishes after publishing. Carry every limit into the selected child skills.
...
5. Follow `babysit-pr` through current-head CI, verified review feedback, and conflicts until its completion or escalation condition is reached. Let that skill determine when waiting for human input is a stopping condition; report pending required approvals as a blocker to merge readiness. Use the host's supported wait or recurring mechanism; describe monitoring as active only while that mechanism or the session is running.
